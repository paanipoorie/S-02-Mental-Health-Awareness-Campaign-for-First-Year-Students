...
export function AdminResourcesClient({ initialData }: { initialData?: ResourceData }) {
  ...
  const fetchResources = async (page: number, searchQuery: string, filterOptions: Record<string, string>) => {
    setLoading(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-heading-24 font-bold text-slate-900">Resource Management</h1>
          <p className="text-copy-14 text-slate-500 mt-1">Manage mental health resources for students</p>
        </div>
      </div>
